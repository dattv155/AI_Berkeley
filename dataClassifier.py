...
def fillRegion(x, y, datum, marked):
    queue = util.Queue()
    queue.push((x, y))
    marked[x][y] = 1
    while not queue.isEmpty():
        ...
        for dx in (-1, 0, 1):
            for dy in (-1, 0, 1):
                next_x = x + dx
                next_y = y + dy
                if 0 <= next_x < DIGIT_DATUM_WIDTH and 0 <= next_y < DIGIT_DATUM_HEIGHT \
                        and datum.getPixel(next_x, next_y) == 0 and marked[next_x][next_y] == 0:
...
def enhancedFeatureExtractorDigit(datum):
    ...
    marked = [[0 for i in range(DIGIT_DATUM_WIDTH)] for j in range(DIGIT_DATUM_HEIGHT)]
    regions = 0

    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if features[(x, y)] == 0 and marked[x][y] == 0:
                regions += 1
                fillRegion(x, y, datum, marked)
